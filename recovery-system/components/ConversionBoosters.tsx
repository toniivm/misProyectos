...
// Social proof notification - shows real-time purchase activity
export function SocialProofNotification() {
  ...
  const names = ['María', 'Lucía', 'Carlos', 'Andrea', 'Miguel', 'Laura', 'David', 'Sara', 'Pablo', 'Elena']
  const cities = ['Madrid', 'Barcelona', 'Valencia', 'Sevilla', 'Bilbao', 'Zaragoza', 'Málaga', 'Alicante']
  const products = CATALOG.slice(0, 4).map(p => ({
  ...
  useEffect(() => {
    const showRandom = () => {
      ...
      setNotification({
        name,
        product: product?.name ?? 'Noctip',
        time: isEs ? `hace ${minutes} min` : `${minutes} min ago`
      })
      setVisible(true)
    ...
    const firstTimer = setTimeout(showRandom, 8000)
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="fixed bottom-24 left-4 z-40 max-w-[280px] rounded-2xl border border-white/[0.1] bg-[#0d1219]/95 p-3.5 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] sm:bottom-20 sm:left-6"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[12px] font-semibold text-[#f2eee7] truncate">
                {notification.name} {isEs ? 'compró' : 'bought'} {notification.product}
              </p>
              <p className="text-[10px] text-[#6b7785]">
                {isEs ? 'Envío gratis' : 'Free shipping'} · {notification.time}
              </p>
            </div>
          </div>
        </motion.div>
