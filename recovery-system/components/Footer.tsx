...
export default function Footer() {
  ...
  return (
    <footer className="relative border-t border-[#e8e2d8] bg-[#f5f0eb]">
      <div className="mx-auto max-w-[1280px] px-4 py-12 sm:py-20 sm:px-6">
        ...
        <div className="grid gap-10 sm:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-12">
          ...
          <div className="sm:col-span-2 lg:col-span-4 space-y-5">
            <div className="flex items-center gap-3">
              <Image
                ...
                sizes="38px"
              />
              <span className="text-[14px] font-extrabold uppercase tracking-[0.2em] text-[#1a1a1a]">Noctip</span>
            </div>
            <p className="max-w-sm text-[12px] sm:text-[13px] leading-[1.7] text-[#6b7785]">
              {isEs
            ...
            <div className="flex items-center gap-2.5">
              <a href="https://www.tiktok.com/@noctip2" target="_blank" rel="noopener noreferrer"
                className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#e8e2d8] bg-white text-[#6b7785] transition-all hover:border-[#10BFD8] hover:text-[#10BFD8] active:scale-95"
                aria-label="TikTok">
          ...
          <div className="sm:col-span-1 lg:col-span-2">
            <div className="mb-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#4a4a4a]">
              ...
              {CATEGORIES.map((cat) => (
                <li key={cat.id}>
                  <Link href={`/${locale}/shop/${cat.slug}`}
                    className="text-[13px] text-[#6b7785] transition-colors hover:text-[#1a1a1a]">
          ...
          <div className="sm:col-span-1 lg:col-span-2">
            ...
            <ul className="space-y-2">
              {[
                ...
                <li key={link.label}>
                  <Link href={link.href} className="text-[13px] text-[#6b7785] transition-colors hover:text-[#1a1a1a]">
                    {link.label}
                  </Link>
                </li>
              ))}
              ...
              {subscribed ? (
                ...
                <form onSubmit={handleSubscribe} className="flex gap-2">
                  <div className="relative flex-1">
                    <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9a9a9a]" />
                    <input
                      type="email"
                      value={email}
                      ...
                      inputMode="email"
                      disabled={loading}
                      className="w-full rounded-xl border border-[#d5d0ca] bg-white py-2.5 pl-9 pr-3 text-[15px] text-[#1a1a1a] placeholder-[#9a9a9a] outline-none transition focus:border-[#10BFD8]/40 focus:ring-2 focus:ring-[#10BFD8]/10 disabled:opacity-50"
                    />
                  </div>
                  <button
                    type="submit"
        ...
        <div className="mt-10 sm:mt-14 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={label} className="flex items-center gap-2.5 rounded-xl border border-[#e8e2d8] bg-white px-3 py-2.5 transition-all hover:border-[#10BFD8]">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#10BFD8]/10">
                <Icon size={13} className="text-[#10BFD8]" />
              </div>
              <div>
                <div className="text-[11px] font-semibold text-[#1a1a1a]">{label}</div>
                <div className="text-[9px] text-[#6b7785]">{sub}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Payment methods */}
        <div className="mt-8">
          <div className="mb-2 text-[10px] text-[#9a9a9a] text-center sm:text-left">
        ...
        {/* Bottom bar */}
        <div className="mt-6 flex flex-col gap-3 border-t border-[#e8e2d8] pt-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-[10px] text-[#9a9a9a]">
            &copy; {new Date().getFullYear()} Noctip&trade;. {isEs ? 'Todos los derechos reservados.' : 'All rights reserved.'}
          </p>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 rounded-lg border border-[#e8e2d8] bg-white px-2.5 py-1">
              <Lock size={10} className="text-[#10BFD8]" />
              <span className="text-[9px] font-medium text-[#6b7785]">{isEs ? 'SSL Seguro' : 'SSL Secure'}</span>
            </div>
            <div className="flex items-center gap-1.5 rounded-lg border border-[#e8e2d8] bg-white px-2.5 py-1">
              <CreditCard size={10} className="text-[#10BFD8]" />
              <span className="text-[9px] font-medium text-[#6b7785]">Stripe Payments</span>
            </div>
          </div>
        </div>
      </div>
